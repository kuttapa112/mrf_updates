...
// ============================================================
// FIXED: Robust SMSBower response parser
// Handles ALL possible response formats from SMSBower API
// ============================================================
...
function extractPhone(raw) {
    const s = extractStr(raw);
    if (!s) return null;
    // If it's just digits (possibly with spaces/dashes), format as phone
...
function parseNumResp(d) {
    ...
    if (typeof d === 'string') {
        ...
        // Known error strings
        if (t.startsWith('NO_NUMBERS') || t.startsWith('BAD_SERVICE') || t.startsWith('BAD_KEY') || t.startsWith('ERROR') || t.startsWith('WRONG_SERVICE')) {
            return { success: false, error: t };
        }

        // Try JSON parse first
        if (t.startsWith('{') || t.startsWith('[')) {
            try { return parseNumResp(JSON.parse(t)); } catch (e) { /* fall through to V1 */ }
        }

        // V1 format: ACCESS_NUMBER:activationId:phoneNumber
        if (t.startsWith('ACCESS_NUMBER:')) {
            const parts = t.split(':');
            if (parts.length >= 3) {
                const aid = parts[1];
                const phone = parts.slice(2).join(':'); // In case phone has extra colons
                const formattedPhone = phone.startsWith('+') ? phone : `+${phone}`;
                console.log(`[SMSBOWER] Parsed V1: id=${aid}, phone=${formattedPhone}`);
                return { success: true, activationId: aid, phoneNumber: formattedPhone };
            }
        }

        // Maybe just a phone number string?
        const digits = t.replace(/[\s\-\(\)\.]/g, '');
        if (/^\d{7,15}$/.test(digits)) {
            console.log(`[SMSBOWER] Parsed raw phone string: ${t}`);
            return { success: true, activationId: null, phoneNumber: digits.startsWith('+') ? digits : `+${digits}` };
        }

        console.log(`[SMSBOWER] Unrecognized string response: "${t}"`);
        return { success: false, error: t };
    }

    // If it's an object response (axios auto-parses JSON)
    if (d && typeof d === 'object' && !Array.isArray(d)) {
        // Try ALL possible field name combinations for activation ID
        const aid = d.activationId || d.activation_id || d.activationIdStr || d.id || d.activation_id_str || null;
        // Try ALL possible field name combinations for phone number
        const phone = d.phoneNumber || d.phone_number || d.phone || d.number || d.tel || d.mobile || d.phoneNum || d.phone_number || null;

        if (aid && phone) {
            const phoneStr = extractPhone(phone);
            if (phoneStr) {
                console.log(`[SMSBOWER] Parsed JSON object: id=${aid}, phone=${phoneStr}`);
                return { success: true, activationId: String(aid), phoneNumber: phoneStr };
            }
        }

        // If only phone found, no ID
        if (phone && !aid) {
            const phoneStr = extractPhone(phone);
            if (phoneStr) {
                console.log(`[SMSBOWER] Parsed JSON (phone only): phone=${phoneStr}`);
                return { success: true, activationId: null, phoneNumber: phoneStr };
            }
        }

        // If only ID found, no phone - try to use it
        if (aid && !phone) {
            console.log(`[SMSBOWER] Parsed JSON (id only, no phone): id=${aid}`);
            return { success: false, error: 'Got activation ID but no phone number', activationId: String(aid) };
        }

        // Check for nested data objects
        const nestedKeys = ['data', 'response', 'result', 'body', 'content'];
        for (const key of nestedKeys) {
            const nested = d[key];
            if (nested) {
                if (typeof nested === 'string') {
                    // If nested is a string, try parsing it
        ...
        // Check if there's a string field that contains ACCESS_NUMBER
        const strKeys = ['response', 'message', 'body', 'data', 'result', 'text', 'content'];
        for (const key of strKeys) {
            if (typeof d[key] === 'string' && d[key].includes('ACCESS_NUMBER')) {
                return parseNumResp(d[key]);
            }
        }

        // If there's a 'success' field that's false
        if (d.success === false || d.error || d.message) {
            const errMsg = d.error || d.message || d.error_message || 'API error';
            return { success: false, error: extractStr(errMsg) };
        }

        // Log the full object for debugging
        console.log(`[SMSBOWER] Unrecognized JSON response:`, JSON.stringify(d));
        return { success: false, error: 'No number in response' };
    }

    // Array response - try first element
    if (Array.isArray(d) && d.length > 0) {
        return parseNumResp(d[0]);
    }

    console.log(`[SMSBOWER] Unknown response type:`, typeof d, d);
    ...
    (function rec(n) {
        ...
        if (Object.prototype.hasOwnProperty.call(n, 'provider_id') && Object.prototype.hasOwnProperty.call(n, 'price')) {
            ...
            if (!Number.isNaN(pi) && !Number.isNaN(pp)) {
                const k = `${pi}:${pp}`;
                if (!s.has(k)) { s.add(k); b.push({ provider_id: pi, price: pp }); }
            }
        }
...
async function fetchTiers(cid, sc = 'wa') {
    ...
    const d = r.data;
    console.log(`[SMSBOWER] getPricesV3 for ${sc}/${cid}:`, typeof d === 'string' ? d.substring(0, 200) : JSON.stringify(d).substring(0, 200));
    let prov = [];
...
async function buyFromProv(cid, prov, sc = 'wa') {
    try {
        ...
        console.log(`[SMSBOWER] buyFromProv URL: ${url}`);
        const r = await axios.get(url, { timeout: 15000 });
        console.log(`[SMSBOWER] buyFromProv raw response:`, JSON.stringify(r.data));
        const p = parseNumResp(r.data);
        console.log(`[SMSBOWER] buyFromProv parsed:`, p);
        if (p.success) return { ...p, provider_id: prov.provider_id, provider_price: prov.price };
...
async function buyRetry(cid, baseUsd, sc = 'wa') {
    for (let i = 0; i < 3; i++) {
        const mp = (baseUsd * (1 + i * 0.05)).toFixed(3);
        try {
            const url = `${SMSBOWER_URL}?api_key=${SMSBOWER_API_KEY}&action=getNumber&service=${sc}&country=${cid}&maxPrice=${mp}`;
            console.log(`[SMSBOWER] buyRetry URL: ${url}`);
            const r = await axios.get(url, { timeout: 15000 });
            console.log(`[SMSBOWER] buyRetry raw response:`, JSON.stringify(r.data));
            const p = parseNumResp(r.data);
            console.log(`[SMSBOWER] buyRetry parsed:`, p);
            if (p.success) return { success: true, activationId: p.activationId, phoneNumber: p.phoneNumber, strategy: 'fallback' };
            if (i < 2) await waitMs(8000);
        } catch (e) { if (i === 2) return { success: false, error: e.message }; await waitMs(8000); }
    }
...
app.post('/api/forgot-password', async (req, res) => {
    try {
        ...
        if (!vEmail(email)) return res.status(400).send('Valid email required');
        const u = await findUser(email);
        if (!u) return res.json({ sent: false });
        const token = crypto.randomBytes(32).toString('hex');
        const exp = new Date(Date.now() + 3600000).toISOString();
        await qR('UPDATE users SET reset_token=$1,reset_token_expires=$2 WHERE id=$3', [token, exp, u.id]);
        const domain = req.get('origin') || '';
        res.json({ sent: true, link: `${domain}/reset-password?token=${token}` });
...
app.post('/api/reset-password', async (req, res) => {
    try {
        ...
        if (!token || !vPass(newPassword)) return res.status(400).send('Invalid');
        const u = await q1('SELECT * FROM users WHERE reset_token=$1 AND reset_token_expires>NOW()', [token]);
        if (!u) return res.status(400).send('Invalid or expired link');
        await updatePass(u.id, newPassword);
        await qR('UPDATE users SET reset_token=NULL,reset_token_expires=NULL WHERE id=$1', [u.id]);
...
app.post('/api/register', async (req, res) => {
    try {
        const n = String(req.body.name || '').trim(), e = sEmail(req.body.email), p = req.body.password;
        if (!n) return res.status(400).send('Name required');
        if (!vEmail(e)) return res.status(400).send('Valid email required');
        if (!vPass(p)) return res.status(400).send('Password min 6 chars');
        if (await findUser(e)) return res.status(400).send('Email exists');
        await createUser(n, e, p); res.json({ success: true });
    } catch (e) { res.status(500).send(safeErr(e)); }
});
...
app.post('/api/change-password', ensureAuth, async (req, res) => {
    try {
        ...
        if (!currentPassword) return res.status(400).send('Current password required');
        if (!vPass(newPassword)) return res.status(400).send('Min 6 chars');
        const u = await findUserById(req.session.userId); if (!u) return res.status(404).send('Not found');
        if (!(await verifyP(currentPassword, u.password)).valid) return res.status(400).send('Wrong password');
        await updatePass(u.id, newPassword); res.send('OK');
    } catch (e) { res.status(500).send(safeErr(e)); }
});
...
app.get('/api/me', ensureAuth, async (req, res) => {
    try {
        ...
        res.json({ id: u.id, name: u.name, email: u.email, balance: u.balance, role: u.role, referralCode: u.referral_code, maskedPassword: '********' });
    } catch { res.status(500).send('Server error'); }
});
...
// ============================================================
// FIXED: Order endpoint - saves phone_number correctly and returns it
// ============================================================
app.post('/api/order', ensureAuth, rateLimit, async (req, res) => {
    ...
    try {
        ...
        console.log(`[ORDER] getBestNum result:`, JSON.stringify(result));

        if (!result.success || !result.phoneNumber) {
            console.log(`[ORDER] FAILED - no number. phoneNumber="${result.phoneNumber}", error="${result.error}"`);
        ...
        console.log(`[ORDER] SUCCESS - orderId=${ins.id}, phone=${phoneToSave}, activationId=${result.activationId}`);

        // Return BOTH id and phone_number so frontend can show it immediately
...
app.post('/api/orders/:id/replace', ensureAuth, async (req, res) => {
    try {
        ...
        if (!r.success || !r.phoneNumber) return res.status(500).send('No replacement');
        const now = new Date();
        await updateOrder(o.id, {
            phone_number: r.phoneNumber, activation_id: r.activationId, otp_received: false, otp_code: null,
            order_status: 'active', created_at: now.toISOString(),
            expires_at: new Date(now.getTime() + 25 * 60000).toISOString(),
            cancel_available_at: new Date(now.getTime() + 60000).toISOString(),
            provider_cost_usd: r.provider_price || 0
        });
...
app.post('/api/orders/:id/complete', ensureAuth, async (req, res) => {
    try {
        const o = await getOrderById(Number(req.params.id)); if (!o) return res.status(404).send('Not found');
        const u = await findUserById(req.session.userId); if (!u || o.user_id !== u.id) return res.status(403).send('Unauthorized');
        if (!o.otp_received) return res.status(400).send('No OTP');
        try { await axios.get(`${SMSBOWER_URL}?api_key=${SMSBOWER_API_KEY}&action=setStatus&id=${o.activation_id}&status=6`, { timeout: 15000 }); } catch {}
        await updateOrder(o.id, { order_status: 'completed', completed_at: new Date().toISOString() }); res.send('OK');
    } catch (e) { res.status(500).send(safeErr(e)); }
});
...
app.post('/api/orders/:id/expire', ensureAuth, async (req, res) => {
    try {
        ...
        const u = await findUserById(req.session.userId); if (!u) return res.status(401).send('Not found');
        if (o.user_id !== u.id && u.role !== 'admin') return res.status(403).send('Unauthorized');
        if (o.order_status === 'active' && !o.otp_received) {
            try { await axios.get(`${SMSBOWER_URL}?api_key=${SMSBOWER_API_KEY}&action=setStatus&id=${o.activation_id}&status=8`, { timeout: 15000 }); } catch {}
            await updateOrder(o.id, { order_status: 'cancelled' });
        } res.send('OK');
    } catch (e) { res.status(500).send(safeErr(e)); }
});
...
app.get('/api/admin/stats', ensureAdmin, async (req, res) => {
    try {
        ...
        res.json({
            todayRevenue: Number(today.rev || 0), todayOrders: Number(today.cnt || 0),
            totalUsers: Number(users.cnt || 0), activeOrders: Number(active.cnt || 0),
            todaySuccessRate: Number(success.total || 0) > 0 ? Math.round(Number(success.ok || 0) / Number(success.total || 0) * 100) : 0,
            totalRevenue: Number(totalRevenue.rev || 0), totalCost: Number(totalCost.cost || 0), totalProfit: Number(totalProfit.profit || 0)
        });
    } catch (e) { res.status(500).send(safeErr(e)); }
});
...
app.post('/api/add-funds', ensureAuth, upload.single('screenshot'), async (req, res) => {
    try {
        const a = parseFloat(req.body.amount); if (!a || a < 150) return res.status(400).send('Min 150 PKR');
        const s = req.file ? req.file.filename : null; if (!s) return res.status(400).send('Screenshot required');
        const u = await findUserById(req.session.userId); if (!u) return res.status(401).send('Not found');
        await qR('INSERT INTO transactions(user_id,user_email,amount,screenshot) VALUES($1,$2,$3,$4)', [req.session.userId, u.email, a, s]); res.send('OK');
    } catch (e) { res.status(500).send(safeErr(e)); }
});

app.use('/uploads', express.static(UPLOAD_DIR));

function playDing() { try { const { exec } = require('child_process'); exec('echo -e "\\a"'); } catch (e) {} }
