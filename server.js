...
async function approveTransaction(txId) {
    ...
    try {
        ...
        await client.query('UPDATE transactions SET status = $1, type = COALESCE(type, $2) WHERE id = $3', ['approved', 'deposit', txId]);
        await client.query(
...
async function cancelTransaction(txId) {
    ...
    try {
        ...
        await client.query('UPDATE transactions SET status = $1, type = COALESCE(type, $2) WHERE id = $3', ['cancelled', 'deposit', txId]);
        await client.query('COMMIT');
    } catch (err) {
        await client.query('ROLLBACK');
        throw err;
...
async function expireOrderAndRefund(orderId) {
    ...
    try {
        ...
        if (order.order_status === 'expired_refunded') {
            await client.query('COMMIT');
            return {
                found: true,
                expired: true,
        ...
        const now = new Date();
        if (order.order_status !== 'active' || order.otp_received || !expiry || now < expiry) {
            await client.query('COMMIT');
            return {
                found: true,
                expired: false,
        ...
        const updatedRes = await client.query('UPDATE orders SET order_status = $1 WHERE id = $2 RETURNING *', [
            'expired_refunded',
            order.id
        ]);
        await client.query('COMMIT');
...
app.get('/api/orders/:orderId', ensureAuth, async (req, res) => {
    try {
        ...
        if (order.order_status === 'active' && !order.otp_received && order.expires_at && new Date() >= new Date(order.expires_at)) {
...
app.post('/api/orders/:orderId/cancel', ensureAuth, async (req, res) => {
    ...
    try {
        ...
        const now = new Date();
        const expiry = order.expires_at ? new Date(order.expires_at) : null;
        if (expiry && now >= expiry && !order.otp_received && order.order_status === 'active') {
            await client.query('ROLLBACK');
...
app.get('/api/orders/:orderId/otp', ensureAuth, async (req, res) => {
    try {
        ...
        if (order.order_status !== 'active') {
            return res.json({ received: false, inactive: true, status: order.order_status });
        }
...
app.post('/api/request-payment', ensureAuth, upload.single('screenshot'), async (req, res) => {
    try {
        ...
        if (!transaction_id || transaction_id.trim().length < 10) return res.status(400).send('Valid Easypaisa Transaction ID is required');
        await queryRun(
            'INSERT INTO payment_requests (user_id, user_email, amount, transaction_id, screenshot, status) VALUES ($1, $2, $3, $4, $5, $6)',
            [req.session.userId, user.email, amount, transaction_id.trim(), req.file ? req.file.filename : null, 'pending']
        );
...
app.post('/api/add-funds', ensureAuth, async (req, res) => {
    try {
        const amount = parseFloat(req.body.amount);
        if (!amount || amount < 150) return res.status(400).send('Minimum amount 150 PKR');
        if (paymentRateLimiter[req.session.userId] && Date.now() - paymentRateLimiter[req.session.userId] < 60000) {
        ...
        await queryRun(
            'INSERT INTO payment_requests (user_id, user_email, amount, transaction_id, screenshot, status) VALUES ($1, $2, $3, $4, $5, $6)',
            [req.session.userId, user.email, amount, transaction_id.trim(), null, 'pending']
        );
        paymentRateLimiter[req.session.userId] = Date.now();
        res.json({ success: true });
    } catch (err) {
...
app.post('/api/admin/payment-requests/:id/approve', ensureAdmin, async (req, res) => {
    ...
    try {
        ...
        await client.query(
            ...
            [paymentRequest.user_id, paymentRequest.user_email, paymentRequest.amount, 'deposit', 'approved', `Approved payment request #${paymentRequest.id}`, paymentRequest.transaction_id, paymentRequest.screenshot]
        );
        await client.query('UPDATE payment_requests SET status = $1 WHERE id = $2', ['approved', paymentRequest.id]);
        await client.query('COMMIT');
        res.json({ success: true });
    } catch (err) {
        await client.query('ROLLBACK');
...
app.post('/api/admin/payment-requests/:id/reject', ensureAdmin, async (req, res) => {
    try {
        const request = await queryOne('SELECT * FROM payment_requests WHERE id = $1', [Number(req.params.id)]);
        if (!request) return res.status(404).send('Payment request not found');
        if (request.status !== 'pending') return res.status(400).send('Only pending payment requests can be rejected');
        await queryRun('UPDATE payment_requests SET status = $1 WHERE id = $2', ['rejected', Number(req.params.id)]);
        res.json({ success: true });
    } catch (err) {
        res.status(400).send(formatSafeError(err, 'Payment request rejection failed'));
    }
});
