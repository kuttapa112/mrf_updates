...
async function updateOrder(id,u){const k=Object.keys(u);if(!k.length)return;const f=k.map((k,i)=>`${k}=$${i+1}`).join(',');const v=k.map(k=>u[k]);v.push(id);await qR(`UPDATE orders SET ${f} WHERE id=$${v.length}`,v);}
...
function extractProv(n,b=new Set()){if(!n||typeof n!=='object')return b;if(Object.prototype.hasOwnProperty.call(n,'provider_id')&&Object.prototype.hasOwnProperty.call(n,'price')){const pi=Number(n.provider_id),pp=Number(n.price);if(!Number.isNaN(pi)&&!Number.isNaN(pp)){const k=`${pi}:${pp}`;if(!b.has(k)){b.add(k);/* push to array via closure */}}}for(const v of Object.values(n)){if(v&&typeof v==='object')extractProv(v,b);}return b;}
function extractProviders(node){const b=[];const s=new Set();(function rec(n){if(!n||typeof n!=='object')return;if(Object.prototype.hasOwnProperty.call(n,'provider_id')&&Object.prototype.hasOwnProperty.call(n,'price')){const pi=Number(n.provider_id),pp=Number(n.price);if(!Number.isNaN(pi)&&!Number.isNaN(pp)){const k=`${pi}:${pp}`;if(!s.has(k)){s.add(k);b.push({provider_id:pi,price:pp,count:n.count});}}}for(const v of Object.values(n)){if(v&&typeof v==='object')rec(v);}})(node);return b.sort((a,b)=>a.price-b.price);}
...
app.post('/api/forgot-password',async(req,res)=>{
    try{
        ...
        if(!u)return res.json({sent:false,msg:'If account exists, reset link generated'});
        const token=crypto.randomBytes(32).toString('hex');
        const expires=new Date(Date.now()+3600000).toISOString();
        await qR('UPDATE users SET reset_token=$1,reset_token_expires=$2 WHERE id=$3',[token,expires,u.id]);
...
app.post('/api/reset-password',async(req,res)=>{
    try{
        ...
        if(!token||!vPass(newPassword))return res.status(400).send('Invalid token or password');
        const u=await q1('SELECT * FROM users WHERE reset_token=$1 AND reset_token_expires>NOW()',[token]);
        if(!u)return res.status(400).send('Invalid or expired reset link');
...
app.get('/auth/google/callback',async(req,res)=>{
    try{
        ...
        if(!code||!state||state!==req.session.google_oauth_state)return res.redirect('/?google_error=invalid_state');
        ...
        const at=tr.data.access_token;if(!at)return res.redirect('/?google_error=no_token');
        const pr=await axios.get('https://www.googleapis.com/oauth2/v2/userinfo',{headers:{Authorization:`Bearer ${at}`},timeout:15000});
        const pf=pr.data;if(!pf||!pf.email)return res.redirect('/?google_error=no_email');
        let u=await findUser(pf.email);if(!u){await createUser(pf.name||pf.email.split('@')[0],pf.email,randP());u=await findUser(pf.email);}
        if(!u)return res.redirect('/?google_error=create_failed');
        if(!u.is_active)return res.redirect('/?google_error=blocked');
        await updateLastLogin(u.id);await updateLoginAttempts(u.id,0);
        req.session.regenerate(e=>{if(e)return res.redirect('/?google_error=session_failed');req.session.userId=u.id;req.session.save(se=>{if(se)return res.redirect('/?google_error=session_save_failed');res.redirect('/');});});
    }catch{res.redirect('/?google_error=oauth_failed');}
});
...
app.post('/api/login',async(req,res)=>{
    try{
        ...
        if(!vEmail(e))return res.status(400).send('Valid email required');if(typeof p!=='string'||!p)return res.status(400).send('Password required');
        ...
        req.session.regenerate(e=>{if(e)return res.status(500).send('Login failed');req.session.userId=u.id;req.session.save(se=>{if(se)return res.status(500).send('Login failed');res.json({success:true});});});
    }catch(e){res.status(500).send(safeErr(e));}
});

app.post('/api/change-password',ensureAuth,async(req,res)=>{
    try{const{currentPassword,newPassword}=req.body;if(typeof currentPassword!=='string'||!currentPassword)return res.status(400).send('Current password required');if(!vPass(newPassword))return res.status(400).send('New password min 6 chars');const u=await findUserById(req.session.userId);if(!u)return res.status(404).send('Not found');const pc=await verifyP(currentPassword,u.password);if(!pc.valid)return res.status(400).send('Incorrect password');await updatePass(u.id,newPassword);res.send('OK');}catch(e){res.status(500).send(safeErr(e));}
});

app.get('/api/me',ensureAuth,async(req,res)=>{
    try{const u=await findUserById(req.session.userId);if(!u)return res.status(401).send('Not found');res.json({id:u.id,name:u.name,email:u.email,balance:u.balance,role:u.role,referralCode:u.referralCode,maskedPassword:'********'});}catch{res.status(500).send('Server error');}
});
...
app.post('/api/order',ensureAuth,rateLimit,async(req,res)=>{
    ...
    try{
        ...
        const now=new Date();const exp=new Date(now.getTime()+25*60*1000).toISOString();const cancelAt=new Date(now.getTime()+60*1000).toISOString();
...
app.post('/api/orders/:id/replace',ensureAuth,async(req,res)=>{
    try{
        ...
        if(o.order_status!=='active')return res.status(400).send('Cannot replace');if(o.otp_received)return res.status(400).send('OTP received');
        ...
        const cfg=serviceConfig[o.service_type];const sc=cfg?cfg.code:SVC_WA;
        const r=await getBestNum(o.country_id,pkrUsd(o.price),sc);
        if(!r.success)return res.status(500).send('No replacement');const now=new Date();
        await updateOrder(o.id,{phone_number:r.phoneNumber,activation_id:r.activationId,otp_received:false,otp_code:null,order_status:'active',created_at:now.toISOString(),expires_at:new Date(now.getTime()+25*60*1000).toISOString(),cancel_available_at:new Date(now.getTime()+60*1000).toISOString(),provider_cost_usd:r.provider_price||0});
        res.send('OK');
    }catch(e){res.status(500).send(safeErr(e));}
});
...
app.post('/api/orders/:id/cancel',ensureAuth,async(req,res)=>{
    const c=await pool.connect();try{
        ...
        const now=new Date();if(now<new Date(o.cancel_available_at)){await c.query('ROLLBACK');return res.status(400).send(`Wait ${Math.ceil((new Date(o.cancel_available_at)-now)/1000)}s`);}
        try{await axios.get(`${SMSBOWER_URL}?api_key=${SMSBOWER_API_KEY}&action=setStatus&id=${o.activation_id}&status=8`,{timeout:15000});}catch{}
...
app.get('/api/orders/:id/otp',ensureAuth,async(req,res)=>{
    try{
        ...
        const now=new Date();if(now>=new Date(o.expires_at)&&!o.otp_received&&o.order_status==='active'){try{await axios.get(`${SMSBOWER_URL}?api_key=${SMSBOWER_API_KEY}&action=setStatus&id=${o.activation_id}&status=8`,{timeout:15000});}catch{}await updateOrder(o.id,{order_status:'cancelled'});return res.json({received:false,expired:true});}
...
app.get('/api/admin/stats',ensureAdmin,async(req,res)=>{
    try{
        const todayRes=await q1("SELECT COALESCE(SUM(price),0) as rev, COUNT(*) as cnt FROM orders WHERE created_at>=CURRENT_DATE");
        const usersRes=await q1("SELECT COUNT(*) as cnt FROM users");
        const activeRes=await q1("SELECT COUNT(*) as cnt FROM orders WHERE order_status='active'");
        const successRes=await q1("SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE otp_received=true) as success FROM orders WHERE created_at>=CURRENT_DATE");
        const totalRev=await q1("SELECT COALESCE(SUM(price),0) as rev FROM orders");
        ...
        res.json({
            todayRevenue:Number(todayRes.rev||0),
            todayOrders:Number(todayRes.cnt||0),
            totalUsers:Number(usersRes.cnt||0),
            activeOrders:Number(activeRes.cnt||0),
            todaySuccessRate:Number(totalRes.total||0)>0?Math.round(Number(totalRes.success||0)/Number(totalRes.total)*100):0,
            totalRevenue:Number(totalRev.rev||0),
...
app.get('/api/admin/resets',ensureAdmin,async(req,res)=>{
    try{const rows=await qA("SELECT id,email,reset_token,reset_token_expires FROM users WHERE reset_token IS NOT NULL AND reset_token_expires>NOW() ORDER BY reset_token_expires DESC");res.json(rows);}catch{res.status(500).send('Server error');}
});
...
initDB().then(()=>{app.listen(PORT,'0.0.0.0',()=>console.log(`Server running on http://0.0.0.0:${PORT}`));}).catch(e=>{console.error('DB init failed:',e);process.exit(1);});
