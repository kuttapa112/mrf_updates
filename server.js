...
app.post('/api/add-funds', ensureAuth, async (req, res) => {
    try {
        ...
        if (!transaction_id || transaction_id.trim().length < 5) return res.status(400).send('Valid Easypaisa Transaction ID is required');
        const order_id = Math.random().toString().slice(2, 20);
        await queryRun(
            'INSERT INTO transactions (user_id, user_email, amount, transaction_id, status) VALUES ($1, $2, $3, $4, $5)',
            [req.session.userId, user.email, amount, transaction_id.trim(), 'pending']
        );
