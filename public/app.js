...
async function completeActiveOrder(orderId) {
    ...
    try {
        ...
        // FIX: Remove order from state immediately (Instant Disappear)
        state.orders = state.orders.filter(order => String(order.id) !== String(targetOrderId));
        renderActiveOrders(state.orders);

        // Refresh info (This might bring the order back if server is slow)
        await refreshUserInfo();

        // FIX: Remove again after refresh (Prevent Reappearing due to server latency)
        state.orders = state.orders.filter(order => String(order.id) !== String(targetOrderId));
        renderActiveOrders(state.orders);
