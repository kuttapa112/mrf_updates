...
function resetPaymentModalState() {
    ...
    if (qs('payment-screenshot-name')) {
        qs('payment-screenshot-name').textContent = 'Upload payment screenshot';
    }
}
...
function renderAdminStats(stats) {
    ...
    if (!container) return;
    const cards = [
        { label: 'Total Deposits', value: formatMoneyPrecise(stats.totalDeposits), tone: 'text-white' },
        { label: 'Total API Cost', value: formatMoneyPrecise(stats.totalApiCost), tone: 'text-white' },
        { label: 'Total Profit', value: formatMoneyPrecise(stats.totalProfit), tone: 'text-emerald-300' }
    ];
    container.innerHTML = cards.map((card) => `
        <div class="rounded-[22px] border border-white/10 bg-slate-950/55 p-4">
            <div class="text-xs uppercase tracking-[0.2em] text-slate-500">${escapeHtml(card.label)}</div>
            <div class="mt-2 text-xl font-semibold ${card.tone}">${escapeHtml(card.value)}</div>
        </div>
    `).join('');
}
...
function renderAdminOrders(orders) {
    ...
    const rows = orders.map((order) => {
        ...
        const providerCostText = formatMoneyPrecise(providerCostValue);
        const profitValue = Number(order.price || 0) - providerCostValue;
        const profitText = formatMoneyPrecise(profitValue);
        return `
            <tr class="border-b border-slate-800/80 align-top last:border-b-0">
                ...
                <td class="px-4 py-3 whitespace-nowrap font-semibold text-slate-200">${escapeHtml(providerCostText)}</td>
...
function syncSidebarPendingPayment(requests) {
    ...
    if (!card || !text) return;
    const pendingRequest = (requests || []).find((request) => String(request.status || '').toLowerCase() === 'pending');
    if (!pendingRequest) {
        card.classList.add('hidden');
        text.textContent = 'Payment Pending: 0 PKR';
        return;
    }
    text.textContent = `Payment Pending: ${formatMoneyPrecise(pendingRequest.amount)}`;
    card.classList.remove('hidden');
}
...
async function loadAdminData() {
    ...
    try {
        ...
        const totalApiCost = orders.reduce((sum, order) => sum + Number(order.provider_cost_pkr || 0), 0);
        const totalProfit = orders.reduce((sum, order) => sum + (Number(order.price || 0) - Number(order.provider_cost_pkr || 0)), 0);
...
function bindStaticEvents() {
    ...
    qs('payment-screenshot-input')?.addEventListener('change', (event) => {
        ...
        qs('payment-screenshot-name').textContent = file ? file.name : 'Upload payment screenshot';
