...
function renderCountries() {
    ...
    container.innerHTML = `
        <div class="overflow-hidden rounded-[24px] border border-white/10 bg-slate-950/55">
            <div class="country-table-head border-b border-slate-800/80 bg-slate-900/90">
                <div>Flag</div>
                <div>Country</div>
        ...
        return `
            <article class="sms-order-card">
                ...
                <div class="sms-order-number mt-3">
                    <div class="text-base font-extrabold tracking-[0.04em] text-white">${escapeHtml(order.phone_number || 'Processing...')}</div>
                ...
                ${lifecycleStatus === 'active' ? `
                    <div class="sms-wait-hint">
                        ...
                        <div class="min-w-0">
                            <div class="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">Waiting for SMS</div>
                            <div class="mt-1 text-xs leading-6 text-slate-300">Keep this order open if you want live OTP tracking in the details modal.</div>
                        </div>
                    </div>
                ` : ''}
                ${order.otp_code ? `
                    <div class="mt-3 rounded-[14px] border border-emerald-400/20 bg-emerald-500/10 px-4 py-3">
                        <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-200">Received OTP</div>
                        <div class="mt-2 break-all text-lg font-extrabold tracking-[0.14em] text-white">${escapeHtml(order.otp_code)}</div>
                    </div>
                ` : ''}
                ...
                <div class="sms-order-actions">
                    <button class="sms-order-btn bg-blue-600 text-white transition hover:bg-blue-500" data-action="view-order" data-order-id="${escapeAttr(order.id)}">View Details</button>
                ...
                <div class="payment-detail-main">
                    ...
                    <div class="mt-4 grid gap-2 sm:grid-cols-2">
                        <div class="rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3">
                            ...
                            <div class="mt-2 text-sm font-semibold text-white">${escapeHtml(formatMoneyPrecise(request.amount))}</div>
                        </div>
                        <div class="rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3">
                            <div class="text-[11px] uppercase tracking-[0.18em] text-slate-500">Transaction ID</div>
                            <div class="mt-2 break-all text-sm font-semibold text-white">${escapeHtml(request.transaction_id || 'Awaiting verification')}</div>
                        </div>
                    </div>
                </div>
