...
function renderTransactionCards(transactions, mode) {
    if (!transactions.length) {
        return renderEmptyState(
            ...
            mode === 'pending'
                ? 'New Easypaisa transaction IDs will appear here for verification and approval.'
                : 'Approved and cancelled transaction requests will appear here once processed.'
        );
    }
    const rows = transactions.map((transaction) => {
        ...
        const transactionReference = transaction.transaction_id || `#${transaction.id}`;
        return `
            <tr class="border-b border-slate-800/80 align-top last:border-b-0">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-slate-300">${escapeHtml(transactionReference)}</td>
                ...
                <td class="px-4 py-3">
                    <div class="flex flex-wrap gap-2">
                        <a class="inline-flex items-center gap-1 rounded-xl border px-3 py-1.5 text-xs font-semibold transition hover:bg-white/10" href="https://easypaisa.com.pk/ticket-check/?ticketNo=${escapeAttr(transaction.transaction_id || '')}" target="_blank" style="text-decoration:none; background:rgba(56,189,248,.15); border-color:rgba(56,189,248,.3); color:#38bdf8;">Verify TXID</a>
                        ${mode === 'pending' ? `
...
function bindStaticEvents() {
    ...
    document.getElementById('display-order-id').innerText = Math.random().toString().slice(2, 20);
    document.getElementById('display-current-date').innerText = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit' });
    qs('addFundsForm').addEventListener('submit', async (event) => {
        ...
        try {
            const payload = {
                amount: Number(qs('payment-amount-input').value || 0),
                transaction_id: event.target.elements.transaction_id.value.trim()
            };
            ...
            await response.json();
            event.target.reset();
            qs('display-amount').innerText = '0 PKR';
            document.getElementById('display-order-id').innerText = Math.random().toString().slice(2, 20);
            document.getElementById('display-current-date').innerText = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit' });
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (action === 'open-payment-modal') {
            ...
            document.getElementById('display-order-id').innerText = Math.random().toString().slice(2, 20);
            document.getElementById('display-current-date').innerText = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit' });
            qs('display-amount').innerText = `${qs('payment-amount-input')?.value || 0} PKR`;
            return;
        }
        if (action === 'logout') {
            await logout();
