...
function refreshPaymentReference() {
    qs('display-order-id').innerText = Math.random().toString(36).slice(2, 12).toUpperCase();
    qs('display-current-date').innerText = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit' });
    qs('display-amount').innerText = formatMoney(qs('payment-amount-input')?.value || 0);
}
...
function resetPaymentModalState() {
    ...
    if (qs('payment-screenshot-name')) {
        qs('payment-screenshot-name').textContent = 'Attach payment proof for faster manual review';
    }
    if (qs('payment-success-amount')) {
        qs('payment-success-amount').textContent = '0 PKR';
    }
    if (qs('payment-success-txid')) {
        qs('payment-success-txid').textContent = '—';
    }
...
function renderAdminOrders(orders) {
    ...
    container.innerHTML = renderAdminTable(
        ['User Email', 'Service', 'Number', 'Total Money', 'SMSBower Cost', 'Earned (Profit)'],
        rows,
        'min-w-[980px]'
    );
}
...
async function loadAdminData() {
    ...
    try {
        ...
        qs('admin-pending-count').textContent = String(totalPendingCount);
        qs('admin-history-count').textContent = String(processedPaymentRequests.length);
        qs('payment-badge').textContent = String(totalPendingCount);
        qs('payment-badge').classList.toggle('hidden', totalPendingCount === 0);
...
function bindStaticEvents() {
    ...
    qs('payment-screenshot-input')?.addEventListener('change', (event) => {
        ...
        qs('payment-screenshot-name').textContent = file ? file.name : 'Attach payment proof for faster manual review';
    });
    qs('addFundsForm').addEventListener('submit', async (event) => {
        event.preventDefault();
        const button = qs('submit-payment-btn');
        setLoading(button, 'Submitting request...');
        try {
            const amount = Number(qs('payment-amount-input').value || 0);
            const transactionId = event.target.elements.transaction_id.value.trim().toUpperCase();
            ...
            if (!response.ok) throw new Error(await response.text());
            await response.json();
            qs('payment-success-amount').textContent = formatMoneyPrecise(amount);
            qs('payment-success-txid').textContent = transactionId || '—';
