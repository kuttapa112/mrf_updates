...
function getFilteredActivationOrders() {
    ...
    if (state.activationFilter === 'waiting') {
        // Hard filter: never show completed, cancelled, or expired_refunded orders in Waiting
        return orders.filter((order) => isWaitingViewOrder(order) && !['completed', 'cancelled', 'expired_refunded'].includes(String(order.order_status || '').toLowerCase()));
    }
