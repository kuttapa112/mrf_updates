...
function formatStatus(status) {
    ...
    if (normalized === 'otp_received') return 'OTP Received';
...
function getStatusTone(status) {
    ...
    if (normalized === 'active' || normalized === 'retry_requested') {
...
function getCancelUnlockText(order) {
    return formatCountdown(order?.cancel_available_at, 'Unlocked');
}
...
function getFilteredActivationOrders() {
    ...
    if (state.activationFilter === 'waiting') {
        return orders.filter((order) => isWaitingOrder(order));
    }
...
function updateActivationSummaryLine() {
    ...
    const waitingOrders = state.orders.filter((order) => isWaitingOrder(order));
...
function refreshInlineOrderCountdowns() {
    ...
    qsa('[data-cancel-order-id]').forEach((button) => {
        ...
        if (!order) return;
        const canCancel = getOrderLifecycleStatus(order) === 'active' && isCancelAvailable(order);
...
function renderActiveOrders(orders) {
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const isInitialWaiting = lifecycleStatus === 'active';
        const waitingLabel = lifecycleStatus === 'retry_requested' ? 'Again OTP Requested' : 'Waiting for SMS';
        const waitingDescription = lifecycleStatus === 'retry_requested'
        ...
        const statusNote = isWaitingOrder(order)
            ...
            : lifecycleStatus === 'otp_received'
                ? 'OTP has arrived. Finish the order or request Again OTP directly from this card.'
        ...
        const actionButtons = order.otp_code
            ? `
                <button class="sms-order-btn" data-action="complete-order" data-order-id="${escapeAttr(order.id)}">Finish Order</button>
            ...
            : isInitialWaiting
                ? `
                    ...
                    <button class="sms-order-btn cancel" data-action="cancel-order" data-order-id="${escapeAttr(order.id)}" data-cancel-order-id="${escapeAttr(order.id)}" ${isCancelAvailable(order) ? '' : 'disabled'}>Cancel and Refund</button>
                `
                : '';
        return `
            <article class="sms-order-card">
                ...
                <div class="sms-order-stats">
                    ...
                    <div class="sms-order-stat">
                        <span>${escapeHtml(isInitialWaiting ? 'Cancel Unlock' : order.otp_code ? 'OTP' : 'Status')}</span>
                        <span${isInitialWaiting ? ` data-cancel-unlock-id="${escapeAttr(order.id)}"` : ''}>${escapeHtml(isInitialWaiting ? getCancelUnlockText(order) : order.otp_code || statusText)}</span>
                    </div>
                </div>
